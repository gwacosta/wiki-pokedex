...
export function InputPesquisaMiticos({ setPokemons }: InputPesquisaProps) {
    ...
    async function enviaPesquisa(data: Inputs) {
        ...
        const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/pokemons/pesquisaMiticos/${data.termo}`)
    ...
    async function buscaMiticos() {
        const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/pokemons/pesquisaGrupo/3`)
        const dados = await response.json()
        setPokemons(dados)
        reset({ termo: "" })
    }
                ...
                <form className="flex-1" onSubmit={handleSubmit(enviaPesquisa)}>
                    ...
                    <div className="relative">
                        ...
                        <input
                            ...
                            placeholder="Qual pokémon você procura? (Nome, Tipo, Habilidade Passiva, Número)"
                            required
                            {...register("termo")}
                        />
